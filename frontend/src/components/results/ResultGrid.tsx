...
const CellValue: React.FC<{ value: any }> = ({ value }) => {
  ...
  return (
    <span
      className={`${styles.cellValue} ${styles[`type${type.charAt(0).toUpperCase()}${type.slice(1)}`]}`}
    >
      {displayValue}
    </span>
  );
              ...
              <div
                ...
                onClick={(e) => {
                  ...
                  if (onRowSelectionChange) {
                    const currentRowIndex = virtualRow.index;
                    
                    if (e.shiftKey && lastSelectedIndexRef.current !== null) {
                      const start = Math.min(lastSelectedIndexRef.current, currentRowIndex);
                      const end = Math.max(lastSelectedIndexRef.current, currentRowIndex);
                      const newSelection = { ...selectedRows };
                      const shouldSelect = !isSelected; // Or always select? Usually shift-click selects range

                      // If we are shift-clicking, we usually want to select the range
                      // based on the state of the clicked row or just select all
                      for (let i = start; i <= end; i++) {
                        newSelection[i] = true;
                      }
                      
                      // Using the updater pattern if possible, but here we construct the object
